...
public partial class FormMain : Form
{
    ...
    // 0x0000 - текуща€ длинна полезной нагрузки (2 байта)
    ...
    private void TimerFrame_Tick(object? sender, EventArgs e)
    {
        ...
        if (capture.Read(frame))
        {
            ...
            // ƒобавл€ем кадр в очередь на отправку
    ...
    private void AddToSend(byte[] data, int frameNumber, ushort plsize)
    {
        ...
        udp.Client.ReceiveBufferSize = 65536 * 1000; // ќб€зательно большие буфера!!!
        udp.Client.SendBufferSize = 65536 * 1000; // ќб€зательно большие буфера!!!
        udp.EnableBroadcast = true;

        for (ushort i = 0; i < all; i++)
        {
            var dataToSend = new byte[LenHeader + payloadSize];
            dataToSend[0] = 0x70; // «аголовок HI
            dataToSend[1] = 0x70; // «аголовок LO
            dataToSend[2] = 0x01; // “ип пакета
            ...
            Array.Copy(data, i * plsize, dataToSend, 13, plsize); // полезна€ нагрузка
